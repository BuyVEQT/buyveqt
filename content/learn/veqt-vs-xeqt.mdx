...
excerpt: "VEQT owns about 60% more companies than XEQT, has slightly outperformed it over five years, and is built by a company owned by its investors. Here's the case, made carefully."
category: "comparison"
difficulty: "beginner"
...
Across every recent window, the two are effectively tied — and where a gap shows up, it has leaned slightly toward VEQT, not XEQT. In calendar 2025 both returned almost exactly the same (about 20.45%); over the past five years VEQT has been a hair ahead. The two move almost in lockstep, so the differences are small by construction. The point isn't the decimal — it's that the conventional wisdom (XEQT's US tilt should win) hasn't shown up in the results.
...
<ComparisonTable
  ...
  rows="Provider Structure | Investor-owned (mutual) | Public company (NYSE: BLK) ;; MER | ~0.20%* | ~0.20% ;; Total Holdings | ~13,700 | ~8,500 ;; Canada Weight | ~30% | ~25% ;; US Weight | ~44% | ~45% ;; Emerging Markets | ~7% | ~5% ;; Global Allocation | Market-cap weighted | Fixed targets ;; Index Family | FTSE / CRSP (broader) | S&P / MSCI ;; Inception | January 2019 | August 2019"
/>
